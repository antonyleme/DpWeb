    ...
    if (props.demand) {
      ...
      for (let i = 0; i < props.demand.products.length; i++) {
        if (parseFloat(props.demand.products[i].pivot.qtd) >= 10) {
          total +=
            parseFloat(props.demand.products[i].promo_price) *
            parseFloat(props.demand.products[i].pivot.qtd);
        } else {
          total +=
            parseFloat(props.demand.products[i].pivot.price) *
            parseFloat(props.demand.products[i].pivot.qtd);
        }
      }
      if (props.demand.status == 'balcony')
        return total
          .toFixed(2)
          .toString()
          .replace('.', ',');

      return (total + props.deliveryTax)
        .toFixed(2)
        .toString()
        .replace('.', ',');
    }
                    ...
                    <>
                      ...
                      <tr>
                        ...
                        <td>
                          R$
                          {product.pivot.qtd >= 10
                            ? (product.pivot.qtd * product.promo_price)
                                .toFixed(2)
                                .toString()
                                .replace('.', ',')
                            : (product.pivot.qtd * product.pivot.price)
                                .toFixed(2)
                                .toString()
                                .replace('.', ',')}
                        </td>
                ...
                <tfoot>
                  {props.demand.neighborhood && (
                    <>
                      ...
                      <tr class="ttu">
                        <td>
                          {props.demand.neighborhood}, {props.demand.street},{' '}
                          {props.demand.number}{' '}
                  ...
                  {props.demand.payment_type == 'delivery' && (
                    <tr class="ttu">
                      <td colspan="2">Troco para</td>
                      <td align="right">
                        R$
                        {parseFloat(props.demand.charge_for)
                          .toFixed(2)
                          .toString()
                          .replace(`.`, `,`)}
                      </td>
            ...
            <Grid container spacing={3} style={{ overflowY: 'hidden' }}>
              ...
              {props.demand.user && (
                <Grid item xs={12}>
                  <div style={{ marginBottom: 15 }}>
                    ...
                    <Typography>{props.demand.user.tel}</Typography>
                  </div>
                </Grid>
              )}
              ...
              <Grid item xs={3}>
                <div style={{ marginBottom: 15 }}>
                  ...
                  <Typography>{props.demand.neighborhood}</Typography>
                </div>
              </Grid>
              <Grid item xs={9}>
                <div style={{ marginBottom: 15 }}>
                  <InputLabel>Rua</InputLabel>
                  <Typography>{props.demand.street}</Typography>
                </div>
              </Grid>
              <Grid item sm={3} xs={12}>
                <div style={{ marginBottom: 15 }}>
                  <InputLabel>Número</InputLabel>
                  <Typography>{props.demand.number}</Typography>
                </div>
              </Grid>
              <Grid item sm={3} xs={12}>
                <div style={{ marginBottom: 15 }}>
                  ...
                  <div style={{ marginBottom: 15 }}>
                    ...
                    <Typography>
                      {props.demand.charge_for
                        ? `R$${parseFloat(props.demand.charge_for)
                            .toFixed(2)
                            .toString()
                            .replace('.', ',')}`
                        : 'Não se aplica'}
            ...
            {
              {
                received: (
                  <div>
                    <Button
                      onClick={() => confirmation('refused')}
                      color="primary"
                    >
                      Recusar pedido
                    </Button>
                    <Button
                      onClick={() => confirmation('accepted')}
                      color="primary"
                      variant="contained"
                    >
                      Aceitar pedido
                ...
                accepted: (
                  <div>
                    <Button
                      onClick={() => confirmation('delivered')}
                      color="primary"
                      variant="contained"
                    >
                      Pedido entregue
